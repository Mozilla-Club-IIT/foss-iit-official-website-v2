...
export default function EventCard({ name, description }: Props) {
    return (
        <div className="h-36 w-full flex rounded-xl bg-white/10">
...
export const EventCardWrapper: FC<{ event: Event; isEven: boolean }> = ({ event, isEven }) => {
    const elements = [
        <EventCard
            key="card"
            name={event.name}
